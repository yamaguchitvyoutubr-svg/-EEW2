...
const playAlertSound = (isEEW: boolean = false) => {
  try {
    ...
    // If EEW, make it faster and more aggressive
    ...
    for (let i = 0; i < repeat; i++) {
      ...
      osc1.frequency.linearRampToValueAtTime(dropFreq, startTime + (interval * 0.6)); // Drop
      
      osc2.frequency.setValueAtTime(baseFreq + 40, startTime); // Detuned
...
export const EarthquakeWidget: React.FC = () => {
  ...
  useEffect(() => {
    // If active EEW, large quake (M6+), or active tsunami, poll details fast (6s)
    let isEmergency = false;

    if (eewData) isEmergency = true;
    if (quakeData && quakeData.magnitude >= 6.0) isEmergency = true;
    if (tsunamiData && !tsunamiData.cancelled) isEmergency = true;

    if (testMode) {
        // In test mode, we don't really poll
    } else {
  ...
  useEffect(() => {
      ...
      if (eewData) {
          ...
          // Check Quake (Scale >= 30 means Shindo 3+)
  ...
  useEffect(() => {
      // If EEW is active, force EEW mode
      ...
      // Rotate between views if no EEW
      const rotation = setInterval(() => {
          setActiveMode(prev => {
              if (prev === 'EARTHQUAKE') return 'TSUNAMI';
              if (prev === 'TSUNAMI') return 'EARTHQUAKE';
              return 'EARTHQUAKE'; // Fallback from EEW
          });
      }, 10000);
      return () => clearInterval(rotation);
  }, [testMode, eewData]); // Added eewData dependency

  // --- Test Simulation ---
  const toggleTestMode = () => {
    if (!testMode) {
      setTestMode(true);
      setActiveMode('EEW'); // Start with EEW for impact
      ...
      setEewData({
          ...
          hypocenter: "NANKAI TROUGH (SIMULATION)",
          cancelled: false,
          isWarning: true
      });

      // Simulate Major Earthquake (Background data)
      setQuakeData({
        type: 'EARTHQUAKE',
        time: new Date().toISOString(),
        hypocenter: "NANKAI TROUGH (SIMULATION)",
      ...
      // Simulate Tsunami Warning (Background data)
      setTsunamiData({
        type: 'TSUNAMI',
        time: new Date().toISOString(),
        cancelled: false,
        ...
        areas: [
            { name: "PACIFIC COAST OF KOCHI PREF", grade: "MajorWarning" }
        ]
      });
  ...
  // Override logic for EEW
  const isEEWActive = !!eewData || activeMode === 'EEW';
  ...
  return (
    <div className={`relative flex flex-col items-center mt-4 w-full max-w-2xl transition-all duration-500 h-28 ${(!quakeData && !tsunamiData && !eewData) ? 'opacity-50' : 'opacity-100'}`}>
      ...
      {/* Header / Controls */}
      <div className="w-full flex justify-between items-end mb-1 px-1 flex-wrap gap-y-2">
          {/* Mode Tabs (Disabled/Hidden during EEW) */}
          <div className="flex gap-2">
            {!isEEWActive && (
                <>
                <button 
                    onClick={() => setActiveMode('EARTHQUAKE')}
                    className={`text-[10px] tracking-widest px-3 py-0.5 rounded-t-sm transition-all ${activeMode === 'EARTHQUAKE' ? 'bg-[#0a0a0a] text-cyan-400 border-t border-x border-slate-700' : 'text-slate-600 hover:text-slate-400'}`}
                >
                    SEISMIC
                </button>
                <button 
                    onClick={() => setActiveMode('TSUNAMI')}
                    className={`text-[10px] tracking-widest px-3 py-0.5 rounded-t-sm transition-all ${activeMode === 'TSUNAMI' ? 'bg-[#0a0a0a] text-cyan-400 border-t border-x border-slate-700' : 'text-slate-600 hover:text-slate-400'}`}
                >
                    TSUNAMI
                </button>
                </>
            ...
            {isEEWActive && (
                 <div className="bg-red-600 text-white text-[10px] font-bold px-3 py-0.5 rounded-t-sm tracking-widest animate-pulse">
                     EMERGENCY ALERT
                 </div>
            )}
          </div>
      ...
      <div className={`w-full bg-[#0a0a0a] border-l-4 pr-6 pl-4 py-3 relative overflow-hidden flex items-center justify-between gap-4 shadow-lg backdrop-blur-md transition-all duration-300 h-28 ${borderColor} ${bgColor} ${shadow}`}>
        ...
        {/* Animated Background Scanline for Alerts */}
        {(isQuakeAlert || isTsunamiActive || isEEWActive) && (
          <div className={`absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -skew-x-12 animate-[shimmer_${isEEWActive ? '0.5s' : '2s'}_infinite]`}></div>
        )}
            ...
            <>
                <div className="flex flex-col items-start z-10 min-w-[100px]">
                    <div className="flex items-center gap-2 mb-1">
                        <div className="w-2 h-2 rounded-full bg-red-500 animate-[ping_0.5s_infinite]"></div>
                        <span className="text-[10px] tracking-[0.2em] font-bold text-red-500 animate-pulse whitespace-nowrap">
                            EMERGENCY
                        </span>
                    </div>
                    <div className="text-[10px] text-red-300/70 font-mono">
                        {eewData ? eewData.time.substring(11, 19) : 'NOW'}
                    </div>
                </div>

                <div className="flex-1 flex flex-col items-center z-10 text-center px-2">
                    <span className="text-[10px] text-red-400/80 tracking-widest uppercase mb-0.5 animate-pulse">
                        EARTHQUAKE EARLY WARNING
                    </span>
                    <div className="flex flex-col items-center">
                        <span className="font-sans font-bold tracking-wider text-2xl leading-tight text-white drop-shadow-[0_0_10px_rgba(255,0,0,0.8)]">
                            {eewData ? eewData.hypocenter : '---'}
                        </span>
                        <span className="text-xs text-red-300 mt-1 font-bold">
                             {eewData?.isWarning ? 'STRONG SHAKING EXPECTED' : 'PRELIMINARY REPORT'}
                        </span>
                    </div>
                </div>

                <div className="flex flex-col items-end z-10 min-w-[80px]">
                    <span className="text-[10px] text-red-500 tracking-widest uppercase mb-0.5">STATUS</span>
                    <div className="text-xl font-bold font-sans italic text-red-500 animate-pulse">
                        ALERT
                    </div>
                </div>
            </>
                ...
                <div className="flex flex-col items-start z-10 min-w-[100px]">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[10px] tracking-[0.2em] font-bold ${isQuakeMajor ? 'text-red-500 animate-pulse' : 'text-slate-500'}`}>
                            {isQuakeAlert ? 'WARNING' : 'MONITORING'}
                        </span>
                    </div>
                    <div className="text-[10px] text-slate-600 font-mono">
                        {quakeData ? quakeData.time.substring(0, 16).replace('T', ' ') : 'NO DATA'}
                    </div>
                </div>

                <div className="flex-1 flex flex-col items-center z-10 text-center">
                    <span className="text-[10px] text-slate-500 tracking-widest uppercase mb-0.5">EPICENTER</span>
                    <span className={`font-sans font-bold tracking-wider text-lg leading-tight ${isQuakeMajor ? 'text-white' : 'text-slate-200'}`}>
                 ...
                 <div className="flex flex-col items-start z-10 min-w-[100px]">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[10px] tracking-[0.2em] font-bold ${isTsunamiMajor ? 'text-purple-400 animate-pulse' : isTsunamiWarning ? 'text-red-500' : 'text-slate-500'}`}>
                            {isTsunamiActive ? 'TSUNAMI ALERT' : 'NO TSUNAMI'}
                        </span>
                    </div>
                    <div className="text-[10px] text-slate-600 font-mono">
                        {tsunamiData ? tsunamiData.time.substring(0, 16).replace('T', ' ') : 'SAFE'}
                    </div>
                </div>
                    ...
                    {isTsunamiActive ? (
                        <>
                            <span className="text-[10px] text-slate-500 tracking-widest uppercase mb-0.5">
                                {tsunamiData?.areas.length} AREA(S) AFFECTED
                            </span>
                            <div className="flex flex-col items-center">
                                ...
                                {tsunamiData && tsunamiData.areas.length > 1 && (
                                    <span className="text-[10px] text-slate-500 mt-0.5">
                                        & {tsunamiData.areas.length - 1} OTHER AREAS
                                    </span>
                                )}
                            </div>
                        </>
                    ) : (
                         <span className="text-slate-600 tracking-widest text-sm font-mono">
